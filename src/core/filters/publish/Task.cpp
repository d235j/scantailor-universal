...
class Task::EncoderThread : public QThread
{
public:
    EncoderThread(QProcess& proc): QThread(), m_proc(proc) {
...
Task::needReprocess() const
{
    ...
    return needDjbzReprocess(m_refDjbzDispatcher.findDjbzForPage(m_pageId));
}
...
bool isDjVuCached(const std::unique_ptr<Params>& params)
{
    const QString djvuFilename = params->djvuFilename();
    const uint djvuSize = params->djvuSize();
    const QDateTime djvuLastChanged = params->djvuLastChanged();
    const Params& old_params = params->outputParams().params();
    if (!djvuFilename.isEmpty() && params->djvuSize() &&
            djvuFilename == old_params.djvuFilename() &&
            djvuSize == old_params.djvuSize() &&
            djvuLastChanged == old_params.djvuLastChanged()) {
        const QFileInfo fi(djvuFilename);
        if (fi.exists() && fi.size() == djvuSize &&
                fi.lastModified() == djvuLastChanged) {
            return true;
        }
    }
    return false;
...
Task::process(TaskStatus const& status, FilterData const& data)
{
    ...
    m_ptrFilter->djVuWidget()->setDocument(nullptr);
    ...
    const bool need_reprocess = needReprocess();
    ...
    if (need_reprocess) {
        ...
        QSet<PageId> to_empty_page_gen;
        QSet<PageId> to_assemble;
        QSet<PageId> to_postprocess;
        std::vector<PageId> all_pages_ordered =  m_ptrFilter->pages()->toPageSequence(PAGE_VIEW).asPageIdVector();
        ...
        for (const PageId& p: dictionary_pages) {
            ...
            } else if (!es.hasBWLayer && !es.hasColorLayer) {
                to_empty_page_gen += p; // empty page
            }

            if (requireReassembling(params) || !isDjVuCached(params)) {
                to_assemble += p;
            }
...
#pragma omp for schedule(dynamic)
        ...
        // djvu page is ready
        emit displayDjVu(m_pageId);

        /************************************
         * Bundle multipage DjVu
         ************************************/

        // try to auto-compose bundled djvu doc
        QString bundled_DjVu_fname;
        assert(!all_pages_ordered.empty());

        bool ready_to_bundle = true;
        for (const PageId& p: all_pages_ordered) {
            if (needPageReprocess(p)) {
        ...
        if (ready_to_bundle) {
            bool save_as = false;
            QString fname = m_ptrSettings->bundledDocFilename();
            if (fname.isEmpty()) {
                save_as = true;

                if (!m_ptrFilter->djVuFilenameSuggestion().isEmpty()) {
                    QFileInfo fi(m_ptrFilter->djVuFilenameSuggestion());
                    fname = fi.baseName() + ".djvu";
                }
                fname = m_out_path + fname;
            }


            if (save_as) {
                // can't use static QFileDialog::getSaveFile()
                QFileDialog dlg(m_ptrFilter->optionsWidget(),
        ...
        if (!bundled_DjVu_fname.isEmpty()) {
            ...
            const QString meta_fname = m_djvu_path + "document.meta";
...
bool
Task::checkReadyToBundle(const std::set<PageId>& pages) const
{
    for (const PageId& p : pages) {
        ...
        const QString djbz_id = params->djbzId();
        DjbzDict dict = m_ptrSettings->djbzDispatcherConst().djbzDict(djbz_id);
        OutputParams const output_params_to_use(*params, djbz_id, dict.revision(), dict.params());
        OutputParams const& output_params_was_used(params->outputParams());
        ...
        QFileInfo fi(params->djvuFilename());
