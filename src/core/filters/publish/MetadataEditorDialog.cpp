...
void MetadataEditorDialog::initDocMeta()
{
    m_knownDocMeta =
    {
    ...
    QDialog(parent)
{
    ...
    m_delegate = new QComboBoxDelegate(m_knownDocMeta, tvDocumentMeta);
    tvDocumentMeta->setItemDelegateForColumn(0, m_delegate);
    if (metadata.isEmpty()) {
        displayDefaultFieldsList(tvDocumentMeta, m_knownDocMeta);
    } else {
        displayMetadata(tvDocumentMeta, metadata, m_knownDocMeta);
    }
}
...
void MetadataEditorDialog::on_btnNewField_clicked()
{
    ...
    do {
        ...
        if (ok && !field_id.isEmpty()) {
            if (IsKnownField(m_knownDocMeta, field_id)) {
                field_id.clear();
    ...
    if (ok) {
        m_knownDocMeta.append({field_id, field_id, 0, 0, ""});
    }
}
...
QMap<QString, QString> MetadataEditorDialog::getMetadata()
{
    ...
    for (int i = 0; i < model->rowCount(); i++ ) {
        ...
        if (!val.isEmpty()) { // empty metadata fields are ignored
            res[model->data(key_index, Qt::UserRole).toString()] = val;
        }
    }
    return res;
}
